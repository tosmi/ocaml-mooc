(*  Displaying a Filesystem Hierarchy (200/320 points)
...
structures. A node in the filesystem is either a simple File, a
Directory that contains a nested filesystem, or a Symlink.  The
latter, as on Unix systems, is a fake file that redirects to another
file. For this, it provides the relative path to this target file. The
path is the list of directory to traverse to get to the target file,
followed by the later. If one has to go a directory up, we use the
common ".." directory name that represents the parent directory.

A filesystem is a list of named nodes. An example filesystem is given
below, in the format that you will have to produce. Don't worry, we'll
break this piece by piece.
...
[ "photos", Dir
  ...
  "indexes", Dir
    ...
    Write a function print_path: string list -> unit that prints a
    relative path (the argument of a Symlink) and pretty prints it as
    shown in the example display, using slashes ('/') as separator.

    As you can see in the example, the depth of a file in the
    filesystem (the number of nested folders that are its ancestors)
    is represented by a sequence of vertical lines. Write a function
    print_file: int -> string -> unit that prints a file name, with
    the given number of "| " in front of it.


    Write a similar function print_symlink: int -> string -> string
    list -> unit that prints the link name, with the given number of
    "| " in front of it, and the relative path (preceded by an arrow "
    -> ").

    Write a similar function print_dir: int -> string -> unit that
    prints the dir name, with the given number of "| " in front of it,
    and the prepended '/'.

    Write a function print_filesystem: filesystem -> unit that
    traverses the filesystem, producing the same display as in the
    example. You will probably need an auxiliary, recursive function,
  ...
  let rec print_filesystem lvl items =
    ...
    | x::xs -> match x with
		  ...
		  print_filesystem lvl xs
	       | (entry, File) ->
		  print_file lvl entry;
		  print_newline();
		  print_filesystem lvl xs
	       | (entry, Symlink path) ->
		  print_symlink lvl entry path;
		  print_newline();
		  print_filesystem lvl xs
  in
...
List.tl (List.rev sym);;
resolve sym path;;

(*

1 -> [tosmi;home] [..;dest;other.txt]
2 -> [home] [dest;other.txt]
