...
Note: This exercise is about exceptions, but it uses the unit type
that is only presented in the next sequence.

1.  For each question, we call both your function and a reference
    ...
    to the exception raised by the reference function.  For this, we
    use the 'a result type given in the prelude.  Define a function
    exec: ('a -> 'b) -> 'a -> 'b result, that calls the given function
    on the given argument, and returns Ok with the result if
    everything went well, and Error with the exception raised, if
    there was one.
...
2.    To be able to provide you with the nice error reports, we use an intermediate type for producing reports, similar to the one given in the prelude.
    ...
    accepted. To display exceptions, you can use the provided
    exn_to_string function.


3.  Then we define random samplers for each type of data that will be
    passed to your function. For a given type 'a, a random sampler
    simply has type unit -> 'a, an imperative function that returns a
    new value of type 'a every time you give it a unit.  Define a
    function test with the following signature.
    ...
    sample, you will exec the user function (the first parameter),
    then exec the reference function, and compare the results. It will
    then return the list containing the 10 comparison messages.
    ...
    list. And since the sampler is an imperative, remember to use let
    ... in constructs if necessary, to force the order of evaluation.
